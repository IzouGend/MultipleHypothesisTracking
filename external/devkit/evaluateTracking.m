function allMets=evaluateTracking(seqmap,resDir,dataDir, inputDetName, isFinal)
...
% read sequence map
seqmapFile=fullfile('3rd_party/devkit/seqmaps',seqmap);
...
%     stI.Xi(find(stI.Xi(:)))=-1;
    ...
    if imCoord        
        fprintf('*** 2D (Bounding Box overlap) ***\n'); printMetrics_chanho(mets); fprintf('\n');
    else
...
for s=allSeq
    ...
    if ~strcmp(seqName, inputDetName)
        continue;
    end
